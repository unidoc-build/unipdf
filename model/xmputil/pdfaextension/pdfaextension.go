...
package pdfaextension ;import (_bb "fmt";_c "github.com/trimmer-io/go-xmp/models/dc";_a "github.com/trimmer-io/go-xmp/models/pdf";_g "github.com/trimmer-io/go-xmp/models/xmp_base";_be "github.com/trimmer-io/go-xmp/models/xmp_mm";_cd "github.com/trimmer-io/go-xmp/xmp";
_ff "reflect";_f "sort";_dg "strings";_d "sync";);func _ac (_da _cd .Model ,_ga *_cd .Namespace ,_cf Property )bool {_dag :=_ff .ValueOf (_da ).Elem ();if _dag .Kind ()==_ff .Ptr {_dag =_dag .Elem ();};_ag :=_dag .Type ();for _gac :=0;_gac < _ag .NumField ();
_gac ++{_gce :=_ag .Field (_gac );_gf :=_gce .Tag .Get ("\u0078\u006d\u0070");if _gf ==""{continue ;};if !_dg .HasPrefix (_gf ,_ga .Name ){continue ;};_gff :=_dg .IndexRune (_gf ,':');if _gff ==-1{continue ;};_gg :=_gf [_gff +1:];if _gg ==_cf .Name {_ggb :=_dag .Field (_gac );
return !_ggb .IsZero ();};};return false ;};func init (){_cd .Register (Namespace ,_cd .XmpMetadata );_cd .Register (SchemaNS );_cd .Register (PropertyNS );_cd .Register (ValueTypeNS );_cd .Register (FieldNS );};func (_fed Schemas )Typ ()_cd .ArrayType {return _cd .ArrayTypeUnordered };
...
func (_abd Properties )MarshalXMP (e *_cd .Encoder ,node *_cd .Node ,m _cd .Model )error {return _cd .MarshalArray (e ,node ,_abd .Typ (),_abd );};const (ValueTypeResourceEvent ValueTypeName ="\u0052\u0065\u0073\u006f\u0075\u0072\u0063\u0065\u0045\u0076\u0065\u006e\u0074";
);

// MakeModel creates or gets a model from document.
func MakeModel (d *_cd .Document )(*Model ,error ){_fg ,_ca :=d .MakeModel (Namespace );if _ca !=nil {return nil ,_ca ;};return _fg .(*Model ),nil ;};

// Typ gets the array type of properties.
func (_cb Properties )Typ ()_cd .ArrayType {return _cd .ArrayTypeOrdered };
...
func (_ddc *Model )Namespaces ()_cd .NamespaceList {return _cd .NamespaceList {Namespace }};
...
func FillModel (d *_cd .Document ,extModel *Model )error {var _ce []*_cd .Namespace ;for _ ,_ffb :=range d .Namespaces (){_ce =append (_ce ,_ffb );};_f .Slice (_ce ,func (_e ,_cag int )bool {return _ce [_e ].Name < _ce [_cag ].Name });for _ ,_ab :=range _ce {switch _ab {case Namespace ,SchemaNS ,PropertyNS ,ValueTypeNS ,FieldNS ,_g .NsXmp ,_c .NsDc :continue ;
default:_de ,_db :=GetSchema (_ab .URI );if !_db {continue ;};_bg :=d .FindModel (_ab );_eb :=*_de ;_eb .Property =Properties {};for _ ,_dd :=range _de .Property {_df :=_ac (_bg ,_ab ,_dd );if _df {_eb .Property =append (_eb .Property ,_dd );};};if len (_eb .Property )==0{continue ;
};var _gc bool ;for _fe ,_fb :=range extModel .Schemas {if _fb .Schema ==_eb .Schema {_gc =true ;extModel .Schemas [_fe ]=_eb ;break ;};};if !_gc {extModel .Schemas =append (extModel .Schemas ,_eb );};};};return nil ;};
...
func GetSchema (namespaceURI string )(*Schema ,bool ){_cfd .RLock ();defer _cfd .RUnlock ();_fd ,_ade :=_ceg [namespaceURI ];return _fd ,_ade ;};

// CanTag implements xmp.Model interface.
func (_cea *Model )CanTag (tag string )bool {_ ,_ed :=_cd .GetNativeField (_cea ,tag );return _ed ==nil ;};func init (){_ceg =map[string ]*Schema {_be .NsXmpMM .URI :&XmpMediaManagementSchema ,"\u0078\u006d\u0070\u0069\u0064\u0071":&XmpIDQualSchema ,_a .NsPDF .URI :&PdfSchema ,"\u0073\u0074\u0045v\u0074":&FieldResourceEventSchema ,"\u0073\u0074\u0056e\u0072":&FieldVersionSchema };
...
func (_dge *Model )GetTag (tag string )(string ,error ){_cg ,_dagd :=_cd .GetNativeField (_dge ,tag );if _dagd !=nil {return "",_bb .Errorf ("\u0025\u0073\u003a\u0020\u0025\u0076",Namespace .GetName (),_dagd );};return _cg ,nil ;};
...
// MarshalXMP implements xmp.Marshaler interface.
func (_af ValueTypes )MarshalXMP (e *_cd .Encoder ,node *_cd .Node ,m _cd .Model )error {return _cd .MarshalArray (e ,node ,_af .Typ (),_af );};

// Properties is a list of properties.
type Properties []Property ;

// UnmarshalXMP implements xmp.Unmarshaler interface.
func (_eag *Schemas )UnmarshalXMP (d *_cd .Decoder ,node *_cd .Node ,m _cd .Model )error {return _cd .UnmarshalArray (d ,node ,_eag .Typ (),_eag );};

// UnmarshalText implements encoding.TextUnmarshaler interface.
func (_aaa *PropertyCategory )UnmarshalText (in []byte )error {switch string (in ){case "\u0069\u006e\u0074\u0065\u0072\u006e\u0061\u006c":*_aaa =PropertyCategoryInternal ;case "\u0065\u0078\u0074\u0065\u0072\u006e\u0061\u006c":*_aaa =PropertyCategoryExternal ;
default:*_aaa =PropertyCategoryUndefined ;};return nil ;};

// UnmarshalXMP implements xmp.Unmarshaler interface.
func (_bgf *Properties )UnmarshalXMP (d *_cd .Decoder ,node *_cd .Node ,m _cd .Model )error {return _cd .UnmarshalArray (d ,node ,_bgf .Typ (),_bgf );};
...
// UnmarshalXMP implements xmp.Unmarshaler interface.
func (_gaa *ValueTypes )UnmarshalXMP (d *_cd .Decoder ,node *_cd .Node ,m _cd .Model )error {return _cd .UnmarshalArray (d ,node ,_gaa .Typ (),_gaa );};

// SetSchema sets the schema into given model.
func (_dee *Model )SetSchema (namespaceURI string ,s Schema ){for _agf :=0;_agf < len (_dee .Schemas );_agf ++{if _dee .Schemas [_agf ].NamespaceURI ==namespaceURI {_dee .Schemas [_agf ]=s ;return ;};};_dee .Schemas =append (_dee .Schemas ,s );};

// MarshalXMP implements xmp.Marshaler interface.
func (_ebb Schemas )MarshalXMP (e *_cd .Encoder ,node *_cd .Node ,m _cd .Model )error {return _cd .MarshalArray (e ,node ,_ebb .Typ (),_ebb );};

// Typ gets array type of the field value types.
func (_gfc ValueTypes )Typ ()_cd .ArrayType {return _cd .ArrayTypeOrdered };
...
func (_bga *Model )SyncFromXMP (d *_cd .Document )error {return nil };
...
);var (_ceg =map[string ]*Schema {};_cfd _d .RWMutex ;);

// RegisterSchema registers schema extension definition.
func RegisterSchema (ns *_cd .Namespace ,schema *Schema ){_cfd .Lock ();defer _cfd .Unlock ();_ceg [ns .URI ]=schema ;};

// IsZero checks if the resources list has no entries.
func (_dgf Schemas )IsZero ()bool {return len (_dgf )==0};
