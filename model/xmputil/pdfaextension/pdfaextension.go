...
package pdfaextension ;import (_g "fmt";_c "github.com/trimmer-io/go-xmp/models/dc";_ab "github.com/trimmer-io/go-xmp/models/pdf";_aa "github.com/trimmer-io/go-xmp/models/xmp_base";_bb "github.com/trimmer-io/go-xmp/models/xmp_mm";_ad "github.com/trimmer-io/go-xmp/xmp";
_ae "reflect";_d "sort";_bg "strings";_a "sync";);const (ValueTypeNameBoolean ValueTypeName ="\u0042o\u006f\u006c\u0065\u0061\u006e";ValueTypeNameDate ValueTypeName ="\u0044\u0061\u0074\u0065";ValueTypeNameInteger ValueTypeName ="\u0049n\u0074\u0065\u0067\u0065\u0072";
...
ValueTypeNameURI ValueTypeName ="\u0055\u0052\u0049";ValueTypeNameVersion ValueTypeName ="\u0056e\u0072\u0073\u0069\u006f\u006e";);func init (){_ad .Register (Namespace ,_ad .XmpMetadata );_ad .Register (SchemaNS );_ad .Register (PropertyNS );_ad .Register (ValueTypeNS );
_ad .Register (FieldNS );};

// Namespaces implements xmp.Model interface.
func (_bgg *Model )Namespaces ()_ad .NamespaceList {return _ad .NamespaceList {Namespace }};

// Model is the pdfa extension metadata model.
type Model struct{Schemas Schemas `xmp:"pdfaExtension:schemas"`;};func (_afa Schemas )Typ ()_ad .ArrayType {return _ad .ArrayTypeUnordered };

// GetTag implements xmp.Model interface.
func (_eef *Model )GetTag (tag string )(string ,error ){_ba ,_gc :=_ad .GetNativeField (_eef ,tag );if _gc !=nil {return "",_g .Errorf ("\u0025\u0073\u003a\u0020\u0025\u0076",Namespace .GetName (),_gc );};return _ba ,nil ;};var (Namespace =_ad .NewNamespace ("\u0070\u0064\u0066\u0061\u0045\u0078\u0074\u0065\u006e\u0073\u0069\u006f\u006e","\u0068\u0074\u0074\u0070\u003a\u002f\u002f\u0077\u0077\u0077\u002e\u0061\u0069\u0069\u006d\u002e\u006f\u0072\u0067\u002f\u0070\u0064\u0066\u0061/\u006e\u0073\u002f\u0065\u0078t\u0065\u006es\u0069\u006f\u006e\u002f",NewModel );
...
func (_abg *Model )SetSchema (namespaceURI string ,s Schema ){for _fb :=0;_fb < len (_abg .Schemas );_fb ++{if _abg .Schemas [_fb ].NamespaceURI ==namespaceURI {_abg .Schemas [_fb ]=s ;return ;};};_abg .Schemas =append (_abg .Schemas ,s );};

// SyncToXMP implements xmp.Model interface.
func (_abb *Model )SyncToXMP (d *_ad .Document )error {return nil };

// FillModel fills in the document XMP model.
func FillModel (d *_ad .Document ,extModel *Model )error {var _cf []*_ad .Namespace ;for _ ,_df :=range d .Namespaces (){_cf =append (_cf ,_df );};_d .Slice (_cf ,func (_aaf ,_ag int )bool {return _cf [_aaf ].Name < _cf [_ag ].Name });for _ ,_bbd :=range _cf {switch _bbd {case Namespace ,SchemaNS ,PropertyNS ,ValueTypeNS ,FieldNS ,_aa .NsXmp ,_c .NsDc :continue ;
default:_dc ,_cg :=GetSchema (_bbd .URI );if !_cg {continue ;};_de :=d .FindModel (_bbd );_aab :=*_dc ;_aab .Property =Properties {};for _ ,_fe :=range _dc .Property {_bf :=_fd (_de ,_bbd ,_fe );if _bf {_aab .Property =append (_aab .Property ,_fe );};};
if len (_aab .Property )==0{continue ;};var _feb bool ;for _db ,_adg :=range extModel .Schemas {if _adg .Schema ==_aab .Schema {_feb =true ;extModel .Schemas [_db ]=_aab ;break ;};};if !_feb {extModel .Schemas =append (extModel .Schemas ,_aab );};};};return nil ;
};

// SyncModel implements xmp.Model interface.
func (_cd *Model )SyncModel (d *_ad .Document )error {return nil };
...
func (_gg *ValueTypes )UnmarshalXMP (d *_ad .Decoder ,node *_ad .Node ,m _ad .Model )error {return _ad .UnmarshalArray (d ,node ,_gg .Typ (),_gg );};

// UnmarshalText implements encoding.TextUnmarshaler interface.
func (_aee *PropertyCategory )UnmarshalText (in []byte )error {switch string (in ){case "\u0069\u006e\u0074\u0065\u0072\u006e\u0061\u006c":*_aee =PropertyCategoryInternal ;case "\u0065\u0078\u0074\u0065\u0072\u006e\u0061\u006c":*_aee =PropertyCategoryExternal ;
default:*_aee =PropertyCategoryUndefined ;};return nil ;};
...
func (_gbe PropertyCategory )MarshalText ()([]byte ,error ){switch _gbe {case PropertyCategoryInternal :return []byte ("\u0069\u006e\u0074\u0065\u0072\u006e\u0061\u006c"),nil ;case PropertyCategoryExternal :return []byte ("\u0065\u0078\u0074\u0065\u0072\u006e\u0061\u006c"),nil ;
case PropertyCategoryUndefined :return []byte (""),nil ;default:return nil ,_g .Errorf ("\u0075\u006ed\u0065\u0066\u0069\u006ee\u0064\u0020p\u0072\u006f\u0070\u0065\u0072\u0074\u0079\u0020c\u0061\u0074\u0065\u0067\u006f\u0072\u0079\u0020\u0076\u0061\u006c\u0075e\u003a\u0020\u0025\u0076",_gbe );
};};

// RegisterSchema registers schema extension definition.
func RegisterSchema (ns *_ad .Namespace ,schema *Schema ){_cgca .Lock ();defer _cgca .Unlock ();_ac [ns .URI ]=schema ;};

// MarshalXMP implements xmp.Marshaler interface.
func (_dbd Schemas )MarshalXMP (e *_ad .Encoder ,node *_ad .Node ,m _ad .Model )error {return _ad .MarshalArray (e ,node ,_dbd .Typ (),_dbd );};

// SyncFromXMP implements xmp.Model interface.
func (_agb *Model )SyncFromXMP (d *_ad .Document )error {return nil };

// MarshalXMP implements xmp.Marshaler interface.
func (_be Properties )MarshalXMP (e *_ad .Encoder ,node *_ad .Node ,m _ad .Model )error {return _ad .MarshalArray (e ,node ,_be .Typ (),_be );};func _fd (_ge _ad .Model ,_ca *_ad .Namespace ,_ff Property )bool {_bd :=_ae .ValueOf (_ge ).Elem ();if _bd .Kind ()==_ae .Ptr {_bd =_bd .Elem ();
};_ded :=_bd .Type ();for _ec :=0;_ec < _ded .NumField ();_ec ++{_dg :=_ded .Field (_ec );_cc :=_dg .Tag .Get ("\u0078\u006d\u0070");if _cc ==""{continue ;};if !_bg .HasPrefix (_cc ,_ca .Name ){continue ;};_ga :=_bg .IndexRune (_cc ,':');if _ga ==-1{continue ;
};_bcf :=_cc [_ga +1:];if _bcf ==_ff .Name {_cgc :=_bd .Field (_ec );return !_cgc .IsZero ();};};return false ;};
...
// UnmarshalXMP implements xmp.Unmarshaler interface.
func (_acf *Schemas )UnmarshalXMP (d *_ad .Decoder ,node *_ad .Node ,m _ad .Model )error {return _ad .UnmarshalArray (d ,node ,_acf .Typ (),_acf );};
...
func (_cdd Properties )Typ ()_ad .ArrayType {return _ad .ArrayTypeOrdered };
